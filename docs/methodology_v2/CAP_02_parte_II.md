...
**`stop_type`** (aggiunto nell'Iterazione 4, chiusura O-3 / M-12 v2 Review CAP-04) — qualificatore della natura strutturale del livello stop_loss pubblicato, dominio $\{\text{structural}, \text{personal}\}$. Per i segnali emessi dal motore, il valore è sempre `structural`: lo stop pubblicato deriva dall'algoritmo di Cap.18.1 di Parte IV, sia che il candidato attivo sia un pivot strutturale, sia che sia il candidato sigma di fallback ($\texttt{stop\_loss}^{(\sigma)} = p_{ref} \pm d_{stop,\sigma} \cdot \hat{\sigma}_{\text{pt}}$) — entrambi appartengono alla famiglia degli stop strutturali del motore. Il valore `personal` è riservato come segnaposto formale del dominio per l'eventuale tracciamento dello stop personale dell'operatore in registri esterni al motore (es. diario operativo), in coerenza con la separazione fra stop strutturale e stop personale dichiarata in Cap.18.3 di Parte IV; esso non viene mai prodotto dal motore nei segnali pubblicati. La distinzione interna fra candidato pivot e candidato sigma resta tracciata nel log di emissione (Cap.10.2) come campo strutturato di diagnostica, e alimenta la metrica di qualità strutturale dello stop in Parte V.
