...
**Algoritmo di selezione di $p_{ref}$.** Sia $\mathcal{P}_{high}(t)$ l'insieme dei pivot high confermati nella sessione corrente e disponibili come feature alla barra $t$, e $\mathcal{P}_{low}(t)$ l'insieme dei pivot low confermati e disponibili. I due insiemi sono aggiornati a ogni barra secondo l'algoritmo di Cap.15.3 di Parte III. Il prezzo di riferimento $p_{ref}$ per il segnale emesso alla barra $t$ è determinato come segue, in funzione della direzione del segnale:

- **Segnale long**: $p_{ref} = \max_{p \in \mathcal{P}_{low}(t)} p$, ovvero il livello del pivot low più recente confermato nella sessione. Il segnale long anticipa un'inversione dal basso: la zona di entry è costruita attorno all'ultimo minimo strutturale confermato.
- **Segnale short**: $p_{ref} = \min_{p \in \mathcal{P}_{high}(t)} p$, ovvero il livello del pivot high più recente confermato nella sessione. Il segnale short anticipa un'inversione dall'alto: la zona di entry è costruita attorno all'ultimo massimo strutturale confermato.

Il livello $p_{ref}$ è sempre un multiplo di 5 punti FIB, poiché i pivot sono prezzi OHLC del FIB e il tick size è 5 punti.

**Criteri di selezione deterministici.** Nel caso in cui più pivot dello stesso tipo siano confermati nella sessione, si seleziona il più recente (quello con il timestamp di conferma maggiore), in quanto incorpora le informazioni strutturali più aggiornate. In caso di parità di timestamp di conferma (situazione che non può verificarsi per costruzione dell'algoritmo frattale di Cap.15.3, che conferma un pivot per volta alla chiusura della barra $t + n_c$), si applica la regola di priorità: il pivot con livello di prezzo più sfavorevole per l'operatore (più alto per long, più basso per short) viene selezionato, in conformità con il principio di conservatività del motore.
...
dove $d_{stop,\sigma}$ è un parametro del cromosoma (dominio: $\mathbb{R}^+$, valore provvisorio $d_{stop,\sigma} = 3$, congelato in Parte V). Questo stop sintetico non è un pivot strutturale confermato; il fatto è esplicitato nel log di emissione e nel payload (flag `stop_type \in \{\text{structural}, \text{synthetic}\}$).
...
**Censoring a destra.** I segnali che raggiungono l'`expired` con causa `posttrigger_timeout` (Cap.7.1 di Parte II) senza aver raggiunto né target_1 né stop_loss sono censurati a destra a $\tau_i = \Delta t_{cromosoma}$ minuti di trading. Il censoring è informativo (non casuale): il timer post-trigger è un parametro del cromosoma, non un evento casuale. In Parte V va verificata l'assunzione di non-informativeness del censoring tramite test di dipendenza tra $\Delta t_{cromosoma}$ e le feature di survival.
...
3. Il numero di oscillazioni del prezzo all'interno del range $[p_{low}, p_{high}]$ nelle ultime $N_{osc}$ barre è maggiore o uguale a $n_{osc,min}$: $\text{oscillazioni}([t_{emission} - N_{osc}, t_{emission}]) \geq n_{osc,min}$. Questa condizione verifica che il range sia "attivo" (il prezzo lo ha percorso più volte) e non un artifact di due pivot isolati. I parametri $N_{osc}$ e $n_{osc,min}$ sono parametri del modello, valori provvisori $N_{osc} = 60$ barre e $n_{osc,min} = 2$ oscillazioni, congelati in Parte V.
...
Il modello di survival di Cap.19 opera in modo identico per il setup trade_range e per il setup directional. L'unica differenza è che il vettore di feature $\tilde{\mathbf{x}}$ include una feature aggiuntiva specifica del trade_range: l'**ampiezza normalizzata del range**

$$x^{(A_{range})} = \frac{A_{range}}{\hat{\sigma}_{\text{pt}}(t_{emission})}$$

espressa in sigma-units FIB, già nel dominio delle feature normalizzate del catalogo (analoga alla distanza target in sigma-units). Questa feature cattura la dimensione relativa del range rispetto alla volatilità corrente: un range di 100 pt in regime di alta volatilità ($\hat{\sigma}_{\text{pt}} = 50$ pt) ha un'ampiezza relativa di 2 sigma; in regime di bassa volatilità ($\hat{\sigma}_{\text{pt}} = 15$ pt) l'ampiezza relativa è circa 6{,}7 sigma.

La stima $\hat{p}_{hit}$ prodotta per il trade_range è la probabilità di raggiungere il bordo opposto del range prima dello stop, condizionata su $\tilde{\mathbf{x}}$ (con la feature aggiuntiva $x^{(A_{range})}$), $T_{residuo}$ e $R_{t_{emission}}$. Il filtro survival-based di Cap.20 si applica al trade_range con la stessa soglia $\tau_{surv}$ (o i suoi valori condizionali per regime).
