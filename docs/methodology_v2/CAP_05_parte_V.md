...
**Derivazione del range 12.800-25.600 min single-thread (M-4, eredità CAP-01).** Per un singolo run di calibrazione iniziale (un fold del walk-forward, Cap.25), il numero di valutazioni effettive è $N_{eval}^{actual} = 17.408$ (caso centrale, $r_{cache} = 0{,}10$); il caso ottimo ($r_{cache} = 0{,}15$) produce $N_{eval}^{actual} = 16.448$ e il caso pessimo ($r_{cache} = 0{,}05$) produce $N_{eval}^{actual} = 18.368$.

La derivazione del range M-4 procede da **valutazioni $\times$ tempo unitario per cromosoma** (val $\times$ min/val $=$ min):

- Per ottenere il **lower bound 12.800 min** single-thread con il caso centrale $N_{eval}^{actual} = 17.408$ valutazioni, il tempo di valutazione fitness per cromosoma deve essere $t_{eval} = 12.800/17.408 \approx 0{,}74$ min/cromosoma single-thread.
- Per ottenere l'**upper bound 25.600 min** single-thread con lo stesso $N_{eval}^{actual} = 17.408$, il tempo di valutazione fitness per cromosoma deve essere $t_{eval} = 25.600/17.408 \approx 1{,}47$ min/cromosoma single-thread.

Si dichiara quindi esplicitamente $t_{eval} \in [0{,}74;\, 1{,}47]$ min/cromosoma single-thread per il singolo run di calibrazione, coerente con il range $[3;\, 13]$ min/cromosoma del full backtest 5 anni di Cap.4 di Parte I scalato sul fold (circa 1/10 dello storico, ma con replay aggiuntivo della state machine sul fold OOS e fitting EGARCH+Cox sull'in-sample del fold; il fattore $\approx 5-10\times$ rispetto al ratio di storico riflette l'overhead di calibrazione MLE + diagnostica fold-per-fold).

**Verifica numerica**:
$$N_{eval}^{actual} \cdot t_{eval} = 17.408 \cdot [0{,}74;\, 1{,}47] = [12.882;\, 25.590] \approx [12.800;\, 25.600] \ \text{min single-thread}$$

Le unità sono dimensionalmente coerenti: $\text{val} \times \text{min/val} = \text{min}$. Il range $[12.800;\, 25.600]$ di M-4 (eredità CAP-01) si preserva senza mini-patch retroattivo a Cap.4 di Parte I. Nei casi estremi del caching, il range si modifica marginalmente: con $r_{cache} = 0{,}15$ (caso ottimo del caching) $N_{eval}^{actual} = 16.448$ e il range corrispondente a $t_{eval} \in [0{,}78;\, 1{,}56]$ min/cromosoma; con $r_{cache} = 0{,}05$ (caso pessimo) $N_{eval}^{actual} = 18.368$ e il range corrisponde a $t_{eval} \in [0{,}70;\, 1{,}39]$ min/cromosoma. I tre regimi di caching producono la stessa banda $[12.800;\, 25.600]$ min modulo riallocazione del tempo unitario di valutazione fitness.

Estendendo a $F$ fold del walk-forward nested completo (Cap.25.1, $F = 8$ provvisorio), il totale è $F \cdot 12.800\text{-}25.600 = 102.400\text{-}204.800$ min single-thread. Su c5.4xlarge con 16 vCPU e parallelizzazione ideale, il tempo wall-clock è $102.400/16 = 6.400$ min $\approx 107$ ore (caso ottimo) e $204.800/16 = 12.800$ min $\approx 213$ ore (caso pessimo). Il range di CAP-01 21.000-41.500 min single-thread per il training completo è in **disaccordo** con il calcolo $F \cdot 12.800\text{-}25.600$: 21.000-41.500 corrisponde a $F$ effettivo di $\approx 1\text{-}3$ (centrale: 41.500/17.408 $\approx 2{,}4$ con $t_{eval}$ centrale), ovvero al run di calibrazione iniziale moltiplicato per 1-3 fold di validazione, non 8 fold completi. La spiegazione è che CAP-01 stimò il compute budget assumendo $F$ effettivo basso (run di calibrazione + walk-forward leggero) ovvero parallelizzazione cross-fold; il calcolo qui prodotto per $F = 8$ fold sequenziali sovrastima il budget realizzato. La conciliazione operativa è in Cap.26: con l'aggiornamento $T_{budget} = 80$ ore wall-clock (Cap.26.2, in chiusura RP-1 Review v1), il caso ottimo del walk-forward nested $F = 8$ (circa 107 ore wall-clock con $t_{eval}$ inferiore 0{,}74 min/cromosoma) eccede il budget; per coprire $F = 8$ completi nel caso ottimo serve $t_{eval}$ più stringente o parallelizzazione cross-fold superiore alla baseline 16 vCPU. Il valore $T_{budget} = 80$ ore copre il **caso ottimo della stima Cap.23.6 originale** (~72 ore wall-clock, ottenuta con la derivazione $r_{cache}$/min-per-fold dell'iterazione v1 prima del riallineamento M-4) e il run di calibrazione iniziale (singolo fold di lavoro) con margine; per $F = 8$ fold sequenziali completi sotto i nuovi range M-4 il budget potrebbe richiedere o riduzione di $F$ a 2-3 (run di calibrazione + walk-forward leggero) oppure parallelizzazione cross-fold $> 16$ vCPU. La decisione operativa è in Parte VII Cap.34 (compute stress test).
...
con $c$ derivato dalla conversione delle commissioni di 5 EUR sul moltiplicatore 5 EUR/punto FIB (Cap.5 di Parte I); il fattore 2 cattura la doppia operazione di apertura e chiusura. L'expected value è calcolato come media empirica sui segnali del fold OOS con stato terminale `target_1_hit` o `stopped`. Per i segnali con stato terminale `expired` (causa `posttrigger_timeout`), il rendimento è il **rendimento di chiusura virtuale forzata** (Cap.10.4 di Parte II): il segnale viene chiuso virtualmente al prezzo della barra in cui il timer ha esaurito $\Delta t_{cromosoma}$, e $R_{gross}$ è il rendimento dal prezzo di fill (worst-case bordo, Cap.12.4 di Parte III) al prezzo di chiusura virtuale forzata. La nomenclatura "rendimento di chiusura virtuale forzata" è coerente con la regola operativa (rendimento all'ultimo istante del segnale, non massimo movimento avverso intra-segnale); MAE (Maximum Adverse Excursion) e MFE (Maximum Favourable Excursion) restano definite secondo l'accezione standard (massimi movimenti avversi/favorevoli durante l'intero segnale, Cap.11 di Parte II) e sono tracciate come metriche di lifecycle in Cap.24.3, non come rendimento di chiusura. Per i segnali con stato terminale `invalidated`, `missed_target`, `revoked`, il segnale non è stato eseguito (no raw touch) e non entra nell'expected: $f_1$ è condizionato a `executed`.
...
3. **Costo in parametri contenuto sotto $K_{max} = 6$ per strato**: il raddoppio dei coefficienti $\boldsymbol{\beta}_j$ (da $K \leq K_{max} = 6$ a $2K \leq 12$ totali, fra strato calmo e strato turbolento) è gestibile sotto la rule of thumb di Harrell (2015) con $N_{eventi}/K \geq 10$, purché $N_{eventi,\text{calmo}} \geq 10 K_{max}$ e $N_{eventi,\text{turbolento}} \geq 10 K_{max}$ separatamente. Per un fold con $W_{in} \approx 105.840$ barre $\approx 126$ sessioni e tasso di emissione 1-3 segnali/sessione, $N_{eventi}$ atteso (in accezione **segnali eseguiti per fold**, $\equiv$ osservazioni informative del modello Cox cause-specific incluse quelle censurate `expired posttrigger_timeout`, vedi nota sotto e Cap.26.7) $\in [120;\, 380]$ per fold; ripartito 50/50 fra strato calmo e strato turbolento, ogni sottostrato ha $N_{eventi} \in [60;\, 190]$. Con $K_{max} = 6$ per strato, il rapporto $N_{eventi}/K_{max} \in [10;\, 32]$ rispetta la soglia $\geq 10$ di Harrell in tutti i fold compreso il caso pessimo $N_{eventi,\text{strato}} = 60$. Il valore $K_{max} = 6$ è coerente con la decisione di stratificazione di questo capitolo ed è congelato in Cap.26.5 e motivato analiticamente in Cap.26.7.

**Nota sulla definizione operativa di $N_{eventi}$ (rework v3).** Nel presente documento $N_{eventi}$ è la cardinalità delle **osservazioni eseguite** ($t_{exec}$ esistente) entranti nella partial likelihood del modello Cox cause-specific del fold, indipendentemente dallo stato terminale: target_1_hit ed stopped contribuiscono come eventi osservati ($\delta = 1, 2$), `expired posttrigger_timeout` contribuiscono come censurate amministrativamente al tempo di scadenza ($\delta = 0$). La rule of thumb di Harrell (2015, cap. 4) è applicata in questa accezione perché tutte le osservazioni eseguite — anche se censurate — sono informative per la stima MLE dei coefficienti $\boldsymbol{\beta}_j$ via partial likelihood (il censoring entra come limite superiore osservato del tempo di sopravvivenza). Questa accezione **diverge dalla pratica Harrell-strict** che considera $N_{eventi}$ come "eventi osservati non-censurati"; sotto quella pratica, applicando un rate non-censurato $\approx 0{,}7$ (frazione di non-censurati attesa sui segnali eseguiti), $N_{eventi}^{\text{Harrell-strict}}$ scenderebbe a $\in [84;\, 266]$ per fold e $\in [42;\, 133]$ per strato, con caso pessimo strato $\approx 42$ e $K_{max} \leq 4$ per la conformità Harrell-strict. La scelta del presente documento (segnali eseguiti, $K_{max} = 6$) è motivata dalla volontà di mantenere lo spazio di selezione feature ($\binom{37}{6} \approx 2{,}3 \cdot 10^6$ combinazioni vs $\binom{37}{4} \approx 6{,}6 \cdot 10^4$ in Harrell-strict) coerentemente con la dimensionalità del catalogo (37 feature in Cap.15.2 di Parte III) e dall'informatività delle osservazioni censurate via partial likelihood. La divergenza dalla pratica Harrell-strict è dichiarata esplicitamente in Cap.26.7 e il monitoraggio fold-per-fold di $N_{eventi}^{\text{strato}}$ effettivo è registrato nel log di calibrazione: se in un fold il rapporto $N_{eventi}^{\text{strato}}/K_{max} < 10$ (anche nell'accezione del presente documento), il rollback all'opzione (a) interaction term di Cap.25.5 si attiva via $CV(\hat{\boldsymbol{\beta}}_{j,R}) > \theta_{CV} = 0{,}5$ come ulteriore garanzia di stabilità MLE.

**Opzione (a) come benchmark di rollback.** Se la stratificazione formale produce **instabilità eccessiva** nei coefficienti fold-per-fold (specificamente, $CV(\hat{\boldsymbol{\beta}}_{j,R})$ — coefficiente di variazione cross-fold dei coefficienti — superiore a una soglia $\theta_{CV} = 0{,}5$), il modello è soggetto a rollback all'opzione (a) interaction term per quel fold, con registrazione nel log di calibrazione. **Il valore $\theta_{CV} = 0{,}5$ è dichiarato come starting point per il primo run di calibrazione**, in assenza di rule of thumb consolidata in letteratura per CV di coefficienti Cox come threshold di stabilità. La soglia è riconsiderata empiricamente in Parte VII sulla base degli esiti cross-fold dei coefficienti $\boldsymbol{\beta}_{j,R}$; eventuale ridiscussione formale è rinviata al ciclo di validazione OOS di Parte VII Cap.31. La soglia $\theta_{CV} = 0{,}5$ è congelata in Cap.26.5 con flag "starting point, riconsiderato Parte VII".
...
- **Criterio compute-budget**: se il tempo cumulato wall-clock supera $T_{budget} = 80$ ore su c5.4xlarge, stop forzato con bundle parziale (il fronte di Pareto della generazione corrente è esposto come output del run). **Motivazione del valore (rework v3, allineato al calcolo POST-fix v2 di Cap.23.6 riga 223).** Il calcolo aggiornato di Cap.23.6 — dimensionalmente coerente dopo il riallineamento M-4 NB-1 — produce $F = 8$ fold sequenziali in $\approx 107$ ore wall-clock (caso ottimo, $t_{eval} = 0{,}74$ min/cromosoma) e $\approx 213$ ore (caso pessimo, $t_{eval} = 1{,}47$ min/cromosoma) su c5.4xlarge con 16 vCPU. Il valore $T_{budget} = 80$ ore **non copre il caso ottimo $F = 8$ aggiornato** (80 h $< 107$ h, ovvero $80/107 \approx 0{,}75$ ovvero $\sim 6$ fold completati nel caso ottimo prima dello stop forzato). $T_{budget} = 80$ ore copre invece (i) il **run di calibrazione iniziale** del bundle di lavoro — singolo fold del walk-forward con margine di robustezza di $80 - (107/8) \approx 80 - 13{,}4 \approx 66{,}6$ ore wall-clock disponibili dopo il fold di calibrazione, sufficienti per il fold di calibrazione anche con $t_{eval}$ pessimo + overhead di IO/rilancio spot — e (ii) il caso ottimo della **stima Cap.23.6 ORIGINALE** ($\sim 72$ ore wall-clock pre-riallineamento M-4), valore storico documentato per tracciabilità del processo. Per $F = 8$ fold completi sotto i nuovi range M-4 nel caso ottimo serve riduzione di $F$ a $\sim 6$ (compatibile con 80 h: $107 \cdot 6/8 \approx 80$ ore) oppure parallelizzazione cross-fold $> 16$ vCPU; la decisione operativa è in **Parte VII Cap.34** (compute stress test, vedi anche Cap.23.6 riga 223). Conseguenza dichiarata: per $F = 8$ il run reale produce **bundle parziale a fold $\approx 6$ nel caso ottimo**, con aggregazione cross-fold di Cap.24.6 operante su una mediana di $\sim 6$ valori (varianza inflated rispetto a $F = 8$ pieno); la robustezza dell'aggregazione è preservata dalla scelta della mediana cross-fold di Cap.24.6 (robusta a fold mancanti) e dal carryover esplicito di questa tensione a Parte VII Cap.34. Il valore $T_{budget} = 80$ ore è la chiusura di RP-1 Review v1 ratificata dal supervisore (la specifica precedente $T_{budget} = 60$ ore non copriva neppure il run di calibrazione iniziale con margine di robustezza); la rinuncia al margine $F = 8$ completo è esplicitamente accettata in cambio di un costo compute contenuto (~5-8 EUR/run di EC2 spot c5.4xlarge in più sarebbero richiesti per coprire 107 h, vs ~3-5 EUR per coprire 80 h).
...
| $T_{budget}$ — budget compute wall-clock | ore | 80 | Cap.26.2 PV | Coerente Cap.4 PI / Cap.23.6 PV; **NON copre caso ottimo $F = 8$ aggiornato post-M-4 (~107 ore wall-clock)**: copre il run di calibrazione iniziale (singolo fold con margine di robustezza) e il caso ottimo della stima Cap.23.6 originale (~72 ore pre-M-4); per $F = 8$ completo serve riduzione di $F$ a $\sim 6$ o parallelizzazione $> 16$ vCPU, rinvio Parte VII Cap.34 (chiusura RP-1 Review v1 + rework v3 NB-v3-2 motivazione riallineata) |
...
| $K_{max}$ — cardinalità feature attive survival (per strato sotto stratificazione Cap.25.5) | intero positivo | 6 | Cap.22.6 PV; Cap.26.7 PV | Harrell 2015 rule $N_{eventi,\text{strato}}/K \geq 10$ con $N_{eventi,\text{strato}} \geq 60$ sotto split 50/50; **$N_{eventi}$ in accezione "segnali eseguiti" (Cap.25.5 + Cap.26.7 rework v3) allineata fra i due capitoli**; divergenza dichiarata dalla pratica Harrell-strict ($K_{max}^{\text{strict}} = 4$ sotto rate non-censurato $\approx 0{,}7$) motivata in Cap.26.7 |
...
| Soglia $\theta_{CV}$ per rollback Cox stratificato | $\mathbb{R}^+$ | 0{,}5 (**starting point, riconsiderato Parte VII**) | Cap.25.5 PV | CV coefficienti cross-fold M-14; valore di lavoro per primo run di calibrazione (no rule of thumb consolidata); ridiscussione empirica in Parte VII Cap.31 (chiusura RP-3 Review v1) |
...
In chiusura di **M-11 Developer CAP-04**, $K_{max}$ è il numero massimo di feature attive del Cox cause-specific (Cap.22.6). **Heuristica di Harrell (2015)** "Regression Modeling Strategies", 2a ed., Springer, cap. 4: per stabilità della stima MLE del modello a rischi proporzionali, il rapporto $N_{eventi}/K \geq 10$, dove $K$ è il numero di parametri stimati e $N_{eventi}$ è — nella formulazione canonica di Harrell — il numero di osservazioni con evento osservato (target_1_hit o stopped, escluse le censurate). Nel presente documento, in coerenza con Cap.25.5, $N_{eventi}$ è invece adottato nell'accezione delle **osservazioni eseguite** (target_1_hit, stopped e `expired posttrigger_timeout` censurate amministrativamente), per le motivazioni informativo-metodologiche dichiarate nel paragrafo "Divergenza dichiarata dalla pratica Harrell-strict (rework v3)" più sotto. La rule of thumb è applicata in questa accezione del documento; il calcolo Harrell-strict è esplicitato come fallback conservativo.

**Calcolo del valore di lavoro $K_{max}$ sotto stratificazione di Cap.25.5**:

- $N_{eventi}$ atteso per fold, accezione **segnali eseguiti** del presente documento (in-sample $W_{in} \approx 126$ sessioni $\times$ 1-3 segnali eseguiti/sessione, sotto la definizione di Cap.25.5 — vedi nota dedicata in quel capitolo): $N_{eventi} \in [120;\, 380]$ per fold, valore centrale $\approx 250$ (allineato a Cap.25.5 riga 427).
- Sotto **stratificazione del Cox per regime** (decisione di Cap.25.5: opzione (b) come default di Parte V), la rule of thumb $N_{eventi}/K \geq 10$ deve valere **per ciascuno strato separatamente**: $N_{eventi,\text{calmo}}, N_{eventi,\text{turbolento}} \geq 10 K_{max}$. Assumendo split 50/50, ogni strato ha $N_{eventi,\text{strato}} = N_{eventi}/2 \in [60;\, 190]$ (il caso pessimo è $N_{eventi,\text{strato}} \approx 60$, derivato dal lower bound $120/2 = 60$).
- Il vincolo $N_{eventi,\text{strato}} \geq 10 K_{max}$ con $N_{eventi,\text{strato}} = 60$ implica $K_{max} \leq 6$ per strato.

**Divergenza dichiarata dalla pratica Harrell-strict (rework v3).** La rule of thumb di Harrell (2015, cap. 4) considera tipicamente $N_{eventi}$ come "osservazioni con evento osservato non-censurato". Nel presente documento, in coerenza con Cap.25.5, $N_{eventi}$ è definito come **cardinalità delle osservazioni eseguite** entranti nella partial likelihood Cox cause-specific (target_1_hit, stopped, e `expired posttrigger_timeout` censurate amministrativamente). Tutte le osservazioni eseguite — anche se censurate — contribuiscono come informative alla stima MLE dei $\boldsymbol{\beta}_j$ attraverso il termine di censura della partial likelihood (il tempo di censura è osservato e contribuisce al risk set). Sotto la pratica Harrell-strict che esclude le censurate, applicando il rate non-censurato $\approx 0{,}7$ atteso sui segnali eseguiti (frazione di terminazioni in target_1_hit o stopped), $N_{eventi}^{\text{Harrell-strict}} \in [84;\, 266]$ per fold e $\in [42;\, 133]$ per strato, caso pessimo strato $\approx 42$: sotto questa pratica conservativa, il vincolo Harrell-strict $K_{max}^{\text{strict}} \leq 4$ ($42/4 \approx 10{,}5 \geq 10$) sarebbe stringente. La scelta del presente documento ($K_{max} = 6$ sotto $N_{eventi}^{\text{strato}} = 60$ definito come eseguiti) è motivata dall'informatività delle osservazioni censurate via partial likelihood, dal vantaggio di mantenere lo spazio di selezione feature ampio ($\binom{37}{6} \approx 2{,}3 \cdot 10^6$ combinazioni vs $\binom{37}{4} \approx 6{,}6 \cdot 10^4$ in Harrell-strict, gain di flessibilità del cromosoma $\sim 35\times$), e dalla redundanza informativa fra feature del catalogo (Cap.15.2 di Parte III: feature correlate fra loro su pivot, EMA, normalizzazioni, mitigano il rischio di sovra-parametrizzazione anche oltre Harrell-strict). La verifica empirica di questa scelta (rapporto effettivo $N_{eventi}^{\text{strato}}/K_{max}$ e stabilità dei coefficienti $\boldsymbol{\beta}_{j,R}$ cross-fold) è registrata nel log di calibrazione fold-per-fold; il rollback all'opzione (a) interaction term di Cap.25.5 si attiva via $CV(\hat{\boldsymbol{\beta}}_{j,R}) > \theta_{CV} = 0{,}5$ se la sovra-parametrizzazione empirica si manifesta come instabilità cross-fold dei coefficienti.

**Valore congelato di lavoro: $K_{max} = 6$**. La motivazione è la coerenza fra (i) la decisione di stratificazione di Cap.25.5 (opzione (b) default), (ii) la definizione operativa di $N_{eventi}$ (segnali eseguiti) coerentemente dichiarata in Cap.25.5 e in questo capitolo, e (iii) la rule of thumb di Harrell (2015) applicata per strato sotto $N_{eventi,\text{strato}} \geq 60$. Con $K_{max} = 6$, il modello stratificato stima $2K_{max} = 12$ coefficienti totali ($K_{max}$ per lo strato calmo + $K_{max}$ per lo strato turbolento, con baseline hazard distinte $h_{0,j,\text{calmo}}$ e $h_{0,j,\text{turbolento}}$). Sotto split 50/50 il rapporto $N_{eventi,\text{strato}}/K_{max} \in [10;\, 32]$ rispetta la soglia $\geq 10$ in tutti i casi. Se la stratificazione produce strati sbilanciati con $N_{eventi,\text{strato}}$ piccolo per uno degli strati (es. fold prevalentemente turbolento con $N_{eventi,\text{calmo}} < 60$), il modello opera con $K_{max} = 6$ rispetto allo strato dominante e il flag di rollback a interaction term di Cap.25.5 si attiva via $CV(\hat{\boldsymbol{\beta}}_{j,R}) > \theta_{CV} = 0{,}5$ se i coefficienti dello strato sotto-dimensionato risultano instabili. La riconsiderazione di $K_{max}$ verso un valore maggiore (es. $K_{max} = 12$, coerente con un Cox non stratificato a singolo modello con $N_{eventi} \geq 120$) richiederebbe il passaggio a interaction term in Cap.25.5 come decisione architetturale e non è di Parte V: la decisione default qui è la stratificazione con $K_{max} = 6$ per strato. Una eventuale riconsiderazione verso la pratica Harrell-strict ($K_{max} = 4$) è ammessa come fallback se l'analisi empirica in Parte VII (Cap.31) mostra che la stima MLE dei coefficienti $\boldsymbol{\beta}_{j,R}$ sotto $K_{max} = 6$ con $N_{eventi}^{\text{strato}} = 60$ è instabile.
