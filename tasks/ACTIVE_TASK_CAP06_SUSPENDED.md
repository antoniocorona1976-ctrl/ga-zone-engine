# [SOSPESO 2026-05-26 dall'Orchestratore — CAP-05 riaperto via Review v3 CONDITIONAL]

**Stato sospensione**: il task CAP-06 v2 rework (finding di Review v1 CAP-06 in coda al file) e' messo in pausa perche' la Review v3 indipendente di CAP-05 ha rivelato 2 BUG REALI di chiusura formale (NB-v3-1 su NB-2 K_max=6 + NB-v3-2 su RP-1 T_budget=80h). CAP-06 v1 eredita da CAP-05 v2 e potrebbe contenere bug strutturali a cascata; il ciclo CAP-06 verra' riaperto dopo PASS definitivo di CAP-05 rework v3, con eventuale revisione delle eredita' del Developer di CAP-06 v2 sui capitoli/parametri che CAP-05 rework v3 modifichera' (Cap.25.5, Cap.26.2, Cap.26.5, Cap.26.7).

**Riferimenti pre-sospensione**:
- ACTIVE_TASK al momento della sospensione: questo file (contenuto integrale sotto).
- Commit `bea513f` [ORCH] CAP-06 checkpoint v1: aggiunta della sezione "Finding di Review v1 di CAP-06 da risolvere (rework v2)" con 3 BUG REALI + 8 NEUTRO approvati dal supervisore.
- Review v1 CAP-06: `reviews/REVIEW_CAP_06_review.md` (commit `5b9bc8d`, verdetto FAIL).
- Documento CAP-06 v1: `docs/methodology_v2/CAP_06_parte_VI.md` (commit `8875f1c`).
- Report v1 CAP-06: `reports/REPORT_CAP_06.md`.

**Ripresa attesa**: l'Orchestratore (sessione successiva o questa stessa) verifica se i fix CAP-05 v3 toccano parametri/cross-ref ereditati da CAP-06 v1, e poi rilancia il Developer per CAP-06 v2 rework con il task originale conservato in questo file.

---

# [STORICO PRE-SOSPENSIONE] TASK ATTIVO: CAP-06 -- Parte VI del documento metodologico v2 (Emissione segnali e lifecycle senza execution)
