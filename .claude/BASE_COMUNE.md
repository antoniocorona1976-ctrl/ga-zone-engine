...
- **Frontmatter documentale (G-15)**: il blocco `tools:` / `model:` nel frontmatter dei file di ruolo eseguiti via `general-purpose` è **documentale**, non enforcement: l'agente `general-purpose` ha comunque accesso ai suoi tool. Quindi "il Planner non committa" è disciplina procedurale, non vincolo tecnico — l'Orchestratore committa il task card. Il claim "`.claude/CLAUDE.md` è caricato nel contesto dei subagenti" è da confermare con un test una-tantum (un general-purpose che riporta se vede il contenuto di CLAUDE.md nel proprio contesto); finché non confermato, trattalo come `[DA-VERIFICARE]`, non come fatto.
