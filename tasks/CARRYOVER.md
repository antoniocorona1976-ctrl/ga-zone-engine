...
### Eredità chiusura SPEC-FUNZ-01-ASSEMBLY (assemblaggio loss-less serie B1..B8) — `ee1ea13`, 2026-06-26

Alla chiusura PASS dell'assemblaggio (merge editoriale NON cieco degli 8 blocchi in `docs/spec_funzionale/SPEC_FUNZ_01.md`, che rimpiazza la v2; v2 archiviata `SPEC_FUNZ_01_v2_storico.md` + tag `spec-funz-01-v2-storico`):

- **Nessun M nuovo emesso.** **M-2** (latenza Telegram, OPEN) e **M-GOV-1** (orario sessione, APERTO) sono **incardinati nell'assemblato nello stesso stato** in cui vivono nei blocchi (M-2 → NFR latenza Sez.6 + premessa gate Sez.8 + dipendenza aperta Sez.10; M-GOV-1 → regola operativa Sez.7 + dipendenza aperta Sez.10), **NON chiusi**: restano OPEN/APERTO/PENDING-empirico (RM-1). Schemi DAPI M-1/M-9/M-10 entrano via B6→Sez.9 verbatim (PROVA-EMPIRICA/CODICE-ESISTENTE), mai ri-asseriti di prima istanza.
- **Debito v2 (6 difetti SOLO-v2) — CHIUSO per subsunzione.** L'assemblato è costruito **dai blocchi corretti**, non dalla v2: la submacchina di posizione Cap.11 arriva corretta da B3→Sez.4; le 4 miscitation v2 (CN-2.1, R-3.7, NFR-8.3, R-10.2) spariscono perché la v2 è rimpiazzata. Reviewer (`4eaa7df` + delta `ee1ea13`) ha verificato la **non-reintroduzione** confrontando con `_v2_storico`. La v2 archiviata conserva i 6 difetti come storico (NON patchata, per scelta: doc destinato all'archivio).
- **Cardine EDGE-PENDING ereditato (B7/B8) intatto**: 0 asserzioni d'esito d'edge nell'assemblato; criteri/soglie dichiarati/provvisori; esito d'edge esclusiva `validator`/FASE-D.
- **DISCREPANZA DI CONTEGGIO B5 = 36, non 35 — RISOLTA (2° micro-pass `74d75c8`, 2026-06-27)**: ricontando i blocchi, Developer e Reviewer (indipendentemente, con grep degli ID distinti) avevano verificato che **`SPEC_FUNZ_01_B5.md` contiene 36 requisiti effettivi** (20 R + 9 CN + 7 NFR = **36**), ma la sua riga di conteggio interno (`:281`) **e** la card **e** l'eredità AUDITFIX («B5=35 invariato») sommavano erroneamente **35**. **Totale assemblato reale = 375** (34+42+63+61+**36**+72+49+18). Su decisione AC, **micro-fix applicato a `SPEC_FUNZ_01_B5.md:281` (35→36)** — sola somma, i 36 ID erano già tutti presenti e mappati 1-a-1 in Sez.7; marcatore B5 originale `5ec899c` resta storico, emendamento coperto da re-review `9b2a10f` (precedente AUDITFIX-01). _Nota storica: i riferimenti «B5=35» nelle entry STATO/CARRYOVER pre-27/06 vanno letti come 36 (l'errore aritmetico è ora corretto alla fonte)._
