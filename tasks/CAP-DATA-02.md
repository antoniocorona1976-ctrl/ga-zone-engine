# TASK CARD CAP-DATA-02 (Parte 9)

Questo file è il task card normativo permanente. Quando la sessione CAP-DATA-02 verrà aperta, il suo contenuto sarà copiato in `tasks/ACTIVE_TASK.md` dall'Orchestratore della nuova sessione (vedi STEP 3 del file `tasks/ORCH_INSTRUCTIONS_CAP-DATA-02.md`).
...
## Input addizionali da committare PRIMA dell'apertura sessione

Per soddisfare il Done criterion #3 ("Tutti gli input autoritativi della sezione INPUT verbatim nel capitolo") e il Gap-5 (test regressione exports), oltre al materiale già presente in `tasks/INDAGINE_DIRECTA_CROSS_INDEX.md`, devono essere committati nel repo PRIMA dell'apertura della nuova sessione i seguenti input:

- **`scripts/export_directa_history_parametric.py`** — script Python di riferimento implementativo per il format dati canonico runtime. Atteso da AC #3 e dalla scaletta §4 (Format dati canonico runtime). Il supervisore lo copierà manualmente da `C:\directa_history_parametric_export_overlay\` al repo.
- **`data/runtime/exports_sample/`** con 2-3 file CSV campione di export storico. Atteso da AC #3 e dal Gap-5 (test di regressione contro exports campione già archiviati). Il supervisore copierà 2-3 file esempio dal PC al repo.
- **`docs/runtime/dapi_port_settings_schema.md`** — schema descrittivo del file `APIPortSettings.txt` (formato riga, campi, modello di sicurezza). **GIÀ CREATO in questa sessione (riallineamento del 2026-05-27, STEP-R3)**. Non più atteso dal supervisore.

L'Orchestratore della nuova sessione, allo STEP 1 del prompt `tasks/ORCH_INSTRUCTIONS_CAP-DATA-02.md`, verifica esplicitamente l'esistenza dei tre input sopra prima di procedere con STEP 2.
