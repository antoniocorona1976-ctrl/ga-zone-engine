...
| `active` | `invalidated` | Prima del raw touch: si verifica condizione di invalidazione strutturale (Parte IV, Cap.15); fra cui esplicitamente $p(t) \leq \texttt{stop\_loss}$ per i long e $p(t) \geq \texttt{stop\_loss}$ per gli short con $t < t_{touch}$ |
