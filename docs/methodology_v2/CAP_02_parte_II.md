...
**`target_2_type`** (aggiunto nell'Iterazione 4, chiusura O-3 / M-12 v2 Review CAP-04) — qualificatore della natura strutturale del livello target_2 pubblicato, dominio $\{\text{structural}, \text{synthetic}\}$. Il valore `structural` indica che target_2 è derivato da un pivot strutturale confermato oltre target_1 (Cap.17.4 di Parte IV, insieme $\mathcal{T}^+_2(t)$ o $\mathcal{T}^-_2(t)$ non vuoto). Il valore `synthetic` indica che target_2 è stato calcolato come livello sintetico $\texttt{target\_2} = \texttt{target\_1} \pm k_{t2} \cdot \hat{\sigma}_{\text{pt}}(t_{emission})$ arrotondato al multiplo di 5 più vicino, in fallback all'assenza di pivot strutturali oltre target_1. Il campo è popolato deterministicamente dall'algoritmo di Cap.17.4 di Parte IV. Il consumer Telegram dell'operatore (Cap.9.2) usa il valore per qualificare la natura del livello pubblicato: un livello `synthetic` ha natura informativa derivata da una regola del modello, non da una struttura confermata del prezzo.
...
**`stop_type`** (aggiunto nell'Iterazione 4, chiusura O-3 / M-12 v2 Review CAP-04; dominio raffinato nell'Iterazione 5, chiusura D-v2-7 / NB-v2-2 v2 Review CAP-04) — qualificatore della natura strutturale del livello stop_loss pubblicato, dominio $\{\text{structural}, \text{synthetic}\}$. Il valore `structural` indica che stop_loss è derivato da un pivot strutturale confermato nella direzione avversa al segnale (Cap.18.1 di Parte IV, candidato $\texttt{stop\_loss}^{(pivot)}$ con $\mathcal{S}^-_{low}(t)$ o $\mathcal{S}^+_{high}(t)$ non vuoto). Il valore `synthetic` indica che stop_loss è stato calcolato come livello sintetico derivato dalla regola di fallback basata su $\hat{\sigma}_{\text{pt}}$: $\texttt{stop\_loss}^{(\sigma)} = p_{ref} \pm d_{stop,\sigma} \cdot \hat{\sigma}_{\text{pt}}(t_{emission})$, in fallback all'assenza di pivot strutturali nella direzione avversa o quando il pivot disponibile viola il vincolo $d_{stop} > b$. Il campo è popolato deterministicamente dall'algoritmo di Cap.18.1 di Parte IV. Il dominio non include valori prodotti dall'operatore: il motore non gestisce stop manuali (fuori scope dal contratto del segnale, vedi separazione fra stop strutturale e stop personale di Cap.18.3 di Parte IV). Il consumer Telegram dell'operatore (Cap.9.2) usa il valore per qualificare la natura del livello pubblicato: un livello `synthetic` ha natura informativa derivata da una regola del modello con $d_{stop,\sigma}$ in unità di $\hat{\sigma}_{\text{pt}}$, non da una struttura confermata del prezzo. La simmetria di dominio con `target_2_type` (entrambi $\{\text{structural}, \text{synthetic}\}$) aumenta l'informatività del payload pubblicato. La distinzione interna fra candidato pivot e candidato sigma resta tracciata anche nel log di emissione (Cap.10.2) come campo strutturato di diagnostica, e alimenta la metrica di qualità strutturale dello stop in Parte V.
...
$$A_{range} = p_{high,range} - p_{low,range} \geq 80\ \text{punti FIB}$$

ovvero l'ampiezza $A_{range}$ del rettangolo di prezzo entro cui il setup opera, definita come differenza fra il pivot high e il pivot low che delimitano il range (Cap.21.1 di Parte IV), non può essere inferiore a 80 punti. La formulazione è quella normativa di Cap.5 di Parte I: l'allineamento esplicito di Cap.6.1 a Cap.5 PI sull'ampiezza $A_{range}$ è introdotto nell'Iterazione 4 (chiusura O-6 v2 Review CAP-04), in luogo della precedente formulazione `|target_1 − stop_loss| ≥ 80 pt` che non era allineata con il riferimento normativo. La definizione operativa di $A_{range}$ è in Cap.21.1 di Parte IV. Il vincolo degli 80 punti è un filtro di emissione, non un parametro libero del motore genetico, ed è applicato a valle della valutazione del cromosoma in coerenza con il punto 4 della dichiarazione di intenti e con quanto stabilito in CAP-01.
...
8. `target_2_type` — qualificatore della natura strutturale del livello target_2 pubblicato, dominio $\{\text{structural}, \text{synthetic}\}$ (aggiunto nell'Iterazione 5, chiusura NB-v2-1 v2 Review CAP-04): permette al consumer mobile di distinguere un target_2 derivato da pivot strutturale confermato (Cap.17.4 di Parte IV, insieme $\mathcal{T}^+_2(t)$ o $\mathcal{T}^-_2(t)$ non vuoto) da un target_2 sintetico calcolato come $\texttt{target\_1} \pm k_{t2} \cdot \hat{\sigma}_{\text{pt}}$. Il valore è popolato deterministicamente dall'algoritmo di Cap.17.4 di Parte IV.
9. `stop_type` — qualificatore della natura strutturale del livello stop_loss pubblicato, dominio $\{\text{structural}, \text{synthetic}\}$ (aggiunto nell'Iterazione 5, chiusura NB-v2-1 v2 Review CAP-04; dominio raffinato in chiusura D-v2-7 / NB-v2-2): permette al consumer mobile di distinguere uno stop_loss derivato da pivot strutturale confermato (Cap.18.1 di Parte IV, candidato $\texttt{stop\_loss}^{(pivot)}$) da uno stop sintetico calcolato come $p_{ref} \pm d_{stop,\sigma} \cdot \hat{\sigma}_{\text{pt}}$ in fallback alla regola sigma. Il valore è popolato deterministicamente dall'algoritmo di Cap.18.1 di Parte IV.

I campi $\Delta t_{cromosoma}$ e $T_{touch}^{max}$ non figurano nel messaggio all'operatore: sono parametri tecnici del modello rilevanti per il log interno (Cap.10) ma non per la decisione operativa dell'operatore, che deve soltanto valutare se entrare al raw touch della zona pubblicata. Il messaggio non contiene istruzioni di gestione attiva della posizione (incrementi, scaling out, take profit anticipato, stop profit), in coerenza con il punto 8 della dichiarazione di intenti che riserva queste decisioni all'operatore. La presenza dei due qualificatori `target_2_type` e `stop_type` permette all'operatore di valutare in lettura mobile la natura strutturale dei livelli pubblicati (un livello `synthetic` ha natura derivata da una regola del modello, non da una struttura confermata del prezzo) senza alcun impatto sulla decisione di ingresso, che resta vincolata al raw touch dell'entry_zone.
