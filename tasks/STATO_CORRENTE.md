...
**Ultimo aggiornamento**: 2026-05-30 — sessione **WEB** (AUDIT-RM-RETRO CAP-DATA-02 **fase WEB chiusa PASS** (Re-Review Iter.3 `20961f4`) — **chiusura PARZIALE: audit NON pienamente chiuso fino al PASS empirico CLI**; trovato e corretto **1 BUG catastrofico**: schema CANDLE invertito `O;H;L;C` in Cap.49 → corretto a `C;L;H;O` coerente col decoder canonico, + 8 finding RM-1/2/3; debito retroattivo RM su perimetro A-D di Parte 9 saldato lato WEB; 8 voci Empirico-CLI in handoff)
