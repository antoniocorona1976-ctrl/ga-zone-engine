...
Esempio numerico (tick FIB = 5 pt). Sessione corrente con due pivot low confermati: pivot a livello 27.300 con $\tau_{conf} = b_{135}$ e pivot a livello 27.250 con $\tau_{conf} = b_{210}$, dove $b_k$ denota l'indice di barra corrispondente al $k$-esimo minuto dall'apertura della sessione 8:00 CET (la corrispondenza orario-barra di 1 minuto è biunivoca: $b_{135}$ corrisponde alle 10:15 e $b_{210}$ alle 11:30, secondo la convenzione di indicizzazione di barra usata anche in Cap.7.4 di Parte II e in Cap.16.6 di questa Parte IV). Segnale long emesso alla barra $b_{240}$ (12:00 CET). Applicando il criterio temporale: $\arg\max\{b_{135},\, b_{210}\} = b_{210}$, dunque $p_{ref} = 27.250$. Il pivot estremo in prezzo (27.300) non è selezionato perché meno recente. Simmetricamente per uno short con due pivot high confermati 27.700 (a $\tau_{conf} = b_{135}$) e 27.750 (a $\tau_{conf} = b_{210}$): $p_{ref} = 27.750$.
...
**Flag `stop_type` nel payload formale (chiusura O-3 / M-12 v2; dominio raffinato in chiusura D-v2-7 / NB-v2-2 v2).** La natura strutturale o sintetica dello stop è esplicitata nel **payload formale del segnale** mediante il campo obbligatorio `stop_type` $\in \{\text{structural}, \text{synthetic}\}$ aggiunto alla tupla $\mathcal{S}$ in Cap.6.1 di Parte II (mini-patch Iterazione 4 di CAP-02; dominio raffinato nell'Iterazione 5 di CAP-02 in coerenza con la decisione di design D-v2-7 dell'Orchestratore: simmetria con il dominio di `target_2_type` e aumento dell'informatività del payload pubblicato verso il consumer Telegram). Per i segnali emessi dal motore, il valore del campo `stop_type` è popolato come `structural` quando `stop_loss` deriva dal candidato pivot — ovvero quando l'insieme $\mathcal{S}^-_{low}(t)$ (per long) o $\mathcal{S}^+_{high}(t)$ (per short) è non vuoto e contiene un pivot che soddisfa il vincolo $d_{stop} > b$ di Cap.18.2 — e come `synthetic` quando `stop_loss` deriva dal candidato sigma di fallback con $d_{stop,\sigma}$, ovvero quando nessun pivot strutturale nella direzione avversa è confermato nella sessione corrente oppure tutti i pivot disponibili violano il vincolo $d_{stop} > b$. Il dominio non include il valore `personal`: il motore non gestisce stop manuali (lo stop personale dell'operatore di Cap.18.3 è fuori scope dal contratto del segnale e non è tracciato nel payload pubblicato). La distinzione fra candidato pivot e candidato sigma è dunque visibile direttamente nel payload pubblicato (tramite `stop_type`) e replicata anche nel log di emissione (Cap.10.2 di Parte II) come campo strutturato di diagnostica, che alimenta la metrica di qualità strutturale dello stop in Parte V.
...
L'operatore può adottare uno stop personale più stretto del livello strutturale pubblicato — in conformità con il punto 2 della dichiarazione di intenti ("inserisco -200 pt di stop") — ma questa decisione è fuori scope dal motore e non modifica il contratto del segnale. La separazione è formalmente dichiarata in Cap.2 di Parte I e in Cap.11 di Parte II; qui si riconferma per completezza della Parte IV. Il dominio del campo `stop_type` nel payload pubblicato è $\{\text{structural}, \text{synthetic}\}$ (Cap.18.1 di questa Parte IV; Cap.6.1 di Parte II): il valore `structural` è prodotto quando lo stop deriva dal candidato pivot, il valore `synthetic` è prodotto quando lo stop deriva dal candidato sigma di fallback con $d_{stop,\sigma}$. Il dominio non include valori prodotti dall'operatore: il motore non gestisce stop manuali (out-of-scope dal contratto). Lo stop personale dell'operatore, se adottato, è materia di registri esterni al motore (es. diario operativo) e non interagisce con il payload del segnale né con il log di emissione.
...
ovvero condizionatamente alle covariate del momento dell'emissione, il tempo di censoring non porta informazione aggiuntiva sui tempi di hit o di stop. L'assunzione è **plausibile a priori** per una ragione strutturale: il timer post-trigger $\Delta t_{cromosoma}$ — che è il timer che genera l'evento di censoring `posttrigger_timeout` per i segnali post-fill secondo Cap.7.1 di Parte II e Cap.19.4 sopra (riga "censurati a destra a $\tau_i = \Delta t_{cromosoma}$") — è un parametro del cromosoma calibrato sul training set, frozen sul fold in-sample e applicato uniformemente a tutti i segnali del fold OOS. La sua scelta non è correlata con le realizzazioni intra-fold dei tempi di evento $T_j$: per i segnali OOS del fold, $\Delta t_{cromosoma}$ è una costante intra-fold determinata ex ante dalla calibrazione del fold in-sample precedente, quindi $T_j \perp\!\!\!\perp C \mid \tilde{\mathbf{x}}_t$ è plausibile per costruzione del walk-forward (Cap.13.3 di Parte III). Tuttavia, l'assunzione **non è verificata empiricamente in Parte IV**: la diagnostica formale è rinviata a Parte V (capitolo di calibrazione e diagnostica survival).
...
**Argomento formale (chiusura O-v2-3).** La probabilità $\hat{p}_{hit}$ è definita in Cap.19.2 come incidenza cumulativa di target_1_hit prima di stopped entro il tempo residuo $T_{residuo}$:

$$\hat{p}_{hit}(\tilde{\mathbf{x}}, T_{residuo}, R) = \int_0^{T_{residuo}} h_1(\tau \mid \tilde{\mathbf{x}}, T_{residuo}) \cdot \hat{S}(\tau \mid \tilde{\mathbf{x}}, T_{residuo})\,d\tau.$$

L'integrando $h_1(\tau \mid \cdot) \cdot \hat{S}(\tau \mid \cdot)$ è non negativo per ogni $\tau \geq 0$, in quanto $h_1$ è una funzione di rischio causa-specifica (non negativa per costruzione del modello di Cox di Cap.19.2) e $\hat{S}$ è una funzione di sopravvivenza (compresa in $[0,1]$). L'integrale di una funzione non negativa è una funzione **monotona non decrescente** del limite superiore di integrazione. Inoltre il cap finito $\Delta t_{cromosoma} \in \{1, \ldots, 1680\}$ minuti di trading (Cap.6.1 di Parte II) implica che $T_{residuo}$ è limitato superiormente e che $h_1(\tau \mid \cdot)$ è integrabile su qualsiasi intervallo finito $[0, T_{residuo}]$ — segue dalla forma esponenziale del rischio di Cap.19.2 e dalla finitezza dei coefficienti stimati. Per $T_{residuo} \to 0^+$ l'intervallo di integrazione si riduce a un insieme di misura zero, e l'integrale di una funzione integrabile su un insieme di misura zero è esso stesso zero. Quindi $\lim_{T_{residuo} \to 0^+} \hat{p}_{hit} = 0$ è una conseguenza diretta della monotonicità dell'integrale e della finitezza del cap, senza bisogno di assunzioni aggiuntive sulla forma di $h_{0,1}$ o sui coefficienti $\boldsymbol{\beta}_1, \gamma_1$. Si veda Klein e Moeschberger (2003) [*Survival Analysis: Techniques for Censored and Truncated Data*, Springer, 2a ed., cap. 2] per la convergenza monotona della funzione di sopravvivenza e dell'incidenza cumulativa sotto rischio competitivo positivo.

Questo produce un **filtro implicito di fine sessione**: nelle ultime barre della sessione, il valore $\hat{p}_{hit}$ cala sotto la soglia $\tau_{surv}$ e la condizione $E_{surv}$ diventa falsa, inibendo l'emissione di segnali.
...
**Definizione algoritmica di oscillazione (chiusura NB-2 v2; notazione raffinata in chiusura O-v2-4 v2).** Si definisce formalmente *oscillazione del prezzo nel range* $[p_{low}, p_{high}]$ il completamento di un **crossing completo del range** dal bordo inferiore al bordo superiore (o viceversa) sulla sequenza chiusa di barre 1-min, secondo la regola seguente.

Sia $\epsilon_{osc}$ una **tolleranza di prossimità ai bordi**, parametro provvisorio del modello con valore di lavoro $\epsilon_{osc} = 5$ pt = 1 tick FIB, congelato in Parte V. (La notazione con il pedice $\epsilon_{osc}$ è introdotta nell'Iterazione 3 di CAP-04 in chiusura di O-v2-4 v2: nello stesso Cap.21.2 il simbolo $\epsilon$ senza pedice è già usato nella condizione 2 della classificazione con significato distinto, $\epsilon = b$ come margine di prossimità dell'attuale prezzo ai bordi del range; il pedice $\epsilon_{osc}$ riserva la lettera dell'algoritmo di conteggio oscillazioni e elimina la collisione di simboli.) Una *barra che tocca il bordo inferiore* alla barra $t_a$ è una barra 1-min chiusa tale che $\mathrm{close}(t_a) \in [p_{low} - \epsilon_{osc},\, p_{low} + \epsilon_{osc}]$; simmetricamente, una *barra che tocca il bordo superiore* è una barra tale che $\mathrm{close}(t_a) \in [p_{high} - \epsilon_{osc},\, p_{high} + \epsilon_{osc}]$. Una *oscillazione completata* è una coppia ordinata di barre $(t_a, t_b)$ con $t_b > t_a$ tale che $t_a$ tocca un bordo e $t_b$ tocca il bordo opposto, e tale che nessuna barra $t_c$ con $t_a < t_c < t_b$ tocchi il bordo opposto a quello di $t_a$ prima di $t_b$. Le due barre $(t_a, t_b)$ definiscono **una sola** oscillazione: tocchi successivi del medesimo bordo dopo $t_a$ e prima di $t_b$ non incrementano il conteggio; analogamente, dopo $t_b$ il conteggio della successiva oscillazione richiede che una nuova barra $t_b' > t_b$ tocchi il bordo opposto a quello di $t_b$.

Formalmente, il conteggio $n_{osc}(t)$ alla barra $t$ sulla finestra $[t - N_{osc} + 1,\, t]$ è calcolato dall'algoritmo seguente, deterministico e causale (il commento sulla cross-session è introdotto in chiusura di O-v2-5 v2):

$$
\begin{aligned}
&\text{\# Cross-session: il ciclo si limita alla sessione corrente; le barre della} \\
&\text{\# sessione precedente sono escluse dal conteggio. L'edge case cross-session} \\
&\text{\# \`e neutralizzato dal warm-up } T_{warmup,\text{norm}} = 100 \geq N_{osc} = 60 \\
&\text{\# (Cap.15.4 di Parte III, Cap.16.2 di Parte IV).} \\
&\text{Input: sequenza chiusa di } \mathrm{close}(s) \text{ per } s \in [t - N_{osc} + 1,\, t-1],\ p_{low},\ p_{high},\ \epsilon_{osc}. \\
&\text{Init: } n_{osc} \leftarrow 0;\ \text{bordo\_corrente} \leftarrow \texttt{NONE}. \\
&\text{Per ogni } s \text{ da } t - N_{osc} + 1 \text{ a } t - 1: \\
&\quad \text{se } \mathrm{close}(s) \in [p_{low} - \epsilon_{osc},\, p_{low} + \epsilon_{osc}]: \text{ tocco} \leftarrow \texttt{LOW}; \\
&\quad \text{altrimenti se } \mathrm{close}(s) \in [p_{high} - \epsilon_{osc},\, p_{high} + \epsilon_{osc}]: \text{ tocco} \leftarrow \texttt{HIGH}; \\
...
L'algoritmo è **deterministico** (passa una sola volta sulla finestra, condizioni di confronto univoche), **causale** (consuma esclusivamente $\mathrm{close}(s)$ per $s \leq t - 1$, in coerenza con $\mathcal{F}_{t-1}$), e produce conteggio **bit-exact** identico fra qualsiasi implementazione conforme. Il caso *intervalli di prossimità sovrapposti* ($\epsilon_{osc} \geq A_{range}/2$) non si verifica per costruzione: la condizione 1 impone $A_{range} \geq 80$ pt e il valore di lavoro $\epsilon_{osc} = 5$ pt implica $\epsilon_{osc} \ll A_{range}/2$ in ogni configurazione ammissibile; il vincolo $\epsilon_{osc} < A_{range}/2$ è dichiarato come ammissibilità implicita del cromosoma quando $\epsilon_{osc}$ sarà congelato in Parte V.

Esempio numerico (tick FIB = 5 pt, $\epsilon_{osc} = 5$ pt; chiusura NB-v2-4 v2: tutti i prezzi multipli di 5). Range $[27.400,\, 27.500]$, $N_{osc} = 60$ barre. Sequenza di close (estratti, tutti multipli di 5 in coerenza con il tick FIB): barra $t-50$ close 27.400 (tocco LOW: $27.400 \in [27.395, 27.405]$), barra $t-45$ close 27.450 (NONE, nel mezzo del range), barra $t-40$ close 27.500 (tocco HIGH: $27.500 \in [27.495, 27.505]$, crossing 1 completato, $n_{osc} = 1$), barra $t-30$ close 27.495 (tocco HIGH: $27.495 \in [27.495, 27.505]$, stesso bordo, no incremento), barra $t-20$ close 27.405 (tocco LOW: $27.405 \in [27.395, 27.405]$, crossing 2 completato, $n_{osc} = 2$), barra $t-10$ close 27.450 (NONE). Conteggio finale $n_{osc}(t) = 2$. Con $n_{osc,min} = 2$, la condizione 3 è soddisfatta. Verifica passo per passo sul pseudocodice: alla barra $t-50$ tocco $= \texttt{LOW}$ e bordo_corrente $= \texttt{NONE}$, quindi bordo_corrente $\leftarrow \texttt{LOW}$ e $n_{osc} = 0$; alla barra $t-45$ tocco $= \texttt{NONE}$, salto; alla barra $t-40$ tocco $= \texttt{HIGH} \neq \texttt{LOW}$, dunque $n_{osc} \leftarrow 1$ e bordo_corrente $\leftarrow \texttt{HIGH}$; alla barra $t-30$ tocco $= \texttt{HIGH} =$ bordo_corrente, nessun incremento; alla barra $t-20$ tocco $= \texttt{LOW} \neq \texttt{HIGH}$, dunque $n_{osc} \leftarrow 2$ e bordo_corrente $\leftarrow \texttt{LOW}$; alla barra $t-10$ tocco $= \texttt{NONE}$, salto. Output $n_{osc} = 2$, coerente con il conteggio dichiarato.
...
**target_2 nel trade_range.** Il target_2 per il trade_range è un livello strutturale oltre il bordo opposto del range: si applica l'algoritmo di selezione di Cap.17.4 partendo da $\texttt{target\_1}$ come riferimento. Se non esistono pivot strutturali confermati oltre il bordo opposto del range, si utilizza il livello sintetico di Cap.17.4 con parametro $k_{t2}$. **Popolamento di `target_2_type` per trade_range (chiusura O-v2-2 v2).** Il campo `target_2_type` del payload (Cap.6.1 di Parte II) viene popolato deterministicamente in coerenza con la regola di Cap.17.4 della presente Parte IV: `structural` quando target_2 deriva da un pivot strutturale confermato oltre il bordo opposto del range, `synthetic` quando deriva dal livello sintetico $\texttt{target\_1} \pm k_{t2} \cdot \hat{\sigma}_{\text{pt}}$. La stessa regola del caso directional di Cap.17.4 si applica al trade_range senza modifiche; la pubblicazione del flag permette al consumer Telegram di qualificare la natura del target_2 anche per i setup trade_range.
...
| $\epsilon_{osc}$ — tolleranza di prossimità ai bordi del range per conteggio oscillazioni | Multipli di 5 pt, $\geq 5$, $< A_{range}/2$ | 5 pt (= 1 tick FIB) | Modello | Cap.21.2 |
