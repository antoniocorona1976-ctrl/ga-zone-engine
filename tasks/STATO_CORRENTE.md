...
**Prossima sessione attesa** (trigger AC "**riprendiamo**", 2026-06-17): **DECISIONE AC SU PROMOZIONE SPEC-FUNZ-01-B3** (State-machine & lifecycle, blocco 3/8). **Stato pronto**: card **rev-B installata e verificata in CLI** (`053ca98`; 46 pin §2 + seam `revoked`/precedenza risolti — vedi `Business Spec/Final/ESITO_B3-CARD-UPDATE-01.md`), **SLOT LIBERO**, `DEV_STATUS` vuoto. **Azione attesa al "riprendiamo"**: su "vai" di AC, invocare **`spec_developer` per B3 in cieco** sulla card rev-B (modalità B: Developer cieco da v2/_v1_storico/B1/B2/B*/chunking; fonte = `CAP_02_parte_II.md` Cap.7 (7.1–7.6) + Cap.11 (11.1–11.5); sede CLI) → **check post-Developer** → **`spec_reviewer` CLI** (audit AC di B3 + confronto-copertura vs v2 `ab7450f` su perimetro lifecycle; il Reviewer può consultare la mappa di chunking come fonte di partizione, F-3) → **chiusura B** (marcatore `SPEC-FUNZ-01-B3: CHIUSO PASS <sha>`). **Opzione alternativa AC** (non bloccante): prima un mini giro-`spec_planner` per ripulire le 2 prose stale della card (`:64` parentetica "pin non risolto a monte", `:214` nota di chiusura sui placeholder ora risolti). _Storico:_ **B2 chiuso PASS 2026-06-15** (`b858a88`); **B1 chiuso PASS** (`7195ffe`); **SPEC-FUNZ-01 v2 chiuso PASS 2026-06-03** (`a16a4c0`). Candidati a fine serie (discrezione AC): proseguire **B4→B8**, **SPEC-FUNZ-02** (PHASE-2 cross-index), avvio **FASE-D**. _Contesto metodologico pregresso (CAP-DATA-03 / Parte 10, invariato):_ versante runtime-tape della convenzione dati chiuso PASS; Audit RM-RETRO CAP-DATA-01/02 CHIUSI (WEB+CLI); residui Empirico-CLI non bloccanti invariati (1030 ticker gated PHASE-2, mesi Mar/Dic, riavvio Darwin mezzanotte).
