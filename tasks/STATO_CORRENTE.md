...
`SPEC-FUNZ-01-B2: CHIUSO PASS 079552c`
...
**Prossima sessione attesa**: **SPEC-FUNZ-01-B3 — State-machine & lifecycle del segnale** (blocco 3/8, ordine B1→B8 deciso da AC; modalità B: Developer cieco dai CAP, confronto-copertura vs v2 `ab7450f` solo in review; sede CLI). Fonte attesa: **CAP-02 Cap.7** (state machine, raw touch come evento, semantica dei timer) ± **Cap.11** (position lifecycle/submacchina, target_2 come evento) — il perimetro esatto lo fissa lo `spec_planner`. **Nessuna dipendenza fragile**: l'SHA di CAP-02 è ora pinnato `a1625df` (G-25 chiuso). Apertura B3 su via del supervisore. _Storico:_ **B2 chiuso PASS 2026-06-15** (review `079552c`); **B1 chiuso PASS** (`7195ffe`); **SPEC-FUNZ-01 v2 chiuso PASS 2026-06-03** (`a16a4c0`). Candidati successivi a fine serie (a discrezione del supervisore): proseguire B4→B8, oppure **SPEC-FUNZ-02** (PHASE-2 cross-index), avvio **FASE-D**. _Storico:_ **SPEC-FUNZ-01 v2 chiuso PASS 2026-06-03** (Re-Review v2 `a16a4c0`). Candidati successivi (a discrezione del supervisore): **SPEC-FUNZ-02** (PHASE-2 cross-index), avvio **FASE-D** (implementazione runtime/training/Telegram/audit), oppure ripresa **Appendici operative** / eventuale **CAP-DATA-04**. _Contesto metodologico pregresso (CAP-DATA-03 / Parte 10):_ Con CAP-DATA-03 / Parte 10 (PASS) si chiude il versante runtime-tape della convenzione dati. Candidati per il prossimo capitolo (a discrezione supervisore/Planner): **Appendici operative** (in particolare **Appendice E** — bot Telegram + **M-2** latenza $L_{max}=30$s, OPEN in CARRYOVER), oppure un eventuale **CAP-DATA-04** (es. calibrazione fine $\theta_{reconcile}$ / temi FASE-D elencati in Parte 10 Cap.64). Prerequisiti empirici DAPI già completi e usati come hard constraint in Parte 10 (V-1 morning+afternoon, V-2 cut-off ~100gg/daily illimitato, T+1 immutabilità). Audit RM-RETRO **CAP-DATA-01 e CAP-DATA-02 entrambi CHIUSI (WEB+CLI)**. Residui Empirico-CLI non bloccanti invariati: 1030 su ticker gated (PHASE-2), mesi Mar/Dic (quando listati), riavvio Darwin mezzanotte (notturno).
