...
## Parte II — Contratto del segnale FIB (~11-12 pp) — IN REVIEW Iterazione 5 (Cap.9.2 esteso a 9 voci pubblicate sul messaggio Telegram: aggiunte voci 8 `target_2_type` e 9 `stop_type` in chiusura di NB-v2-1 v2 Review CAP-04; dominio `stop_type` raffinato a `{structural, synthetic}` in chiusura di D-v2-7 / NB-v2-2 v2)
...
## Parte IV — Geometria zone, target strutturali, survival (~12 pp) — IN REVIEW v3 (rework post-CONDITIONAL v2: 3 BUG REALI chiusi — NB-v2-1 Cap.9.2 esteso, NB-v2-3 razionale (a) di censoring rimosso, NB-v2-4 esempio numerico tick FIB; 1 MIGLIORA PERFORMANCE D-v2-7 — dominio `stop_type` raffinato a `{structural, synthetic}`; 5 NEUTRO inclusi come fix opportunistici — O-v2-1 indici di barra in Cap.16.1, O-v2-2 `target_2_type` esplicitato in Cap.21.4, O-v2-3 formalizzazione $\lim p_{hat,hit} = 0$ in Cap.20.4, O-v2-4 rinominazione $\epsilon_{osc}$ in Cap.21.2, O-v2-5 commento cross-session in pseudocodice oscillazione)
