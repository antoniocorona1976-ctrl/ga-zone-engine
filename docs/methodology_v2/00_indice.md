...
## Parte II — Contratto del segnale FIB (~11-12 pp) — IN REVIEW Iterazione 4 (mini-patch Cap.6.1: campi `target_2_type` e `stop_type` aggiunti al payload formale; formulazione 80pt trade_range sincronizzata con Cap.5 PI come `$A_{range} \geq 80$ pt`)
...
## Parte IV — Geometria zone, target strutturali, survival (~12 pp) — IN REVIEW v2 (rework post-CONDITIONAL: BUG REALI NB-1 criterio temporale `p_ref` + NB-2 definizione formale oscillazione $\epsilon = 5$ pt; PROMEMORIA approvati O-3 flag payload + O-4 catalogo 37 invariato + O-5 censoring non-informativo formalizzato + O-6 80pt sincronizzato Cap.6.1 PII)
