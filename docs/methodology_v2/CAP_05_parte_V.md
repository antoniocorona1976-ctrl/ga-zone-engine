...
- **Criterio compute-budget**: se il tempo cumulato wall-clock supera $T_{budget} = 80$ ore su c5.4xlarge, stop forzato con bundle parziale (il fronte di Pareto della generazione corrente è esposto come output del run). $T_{budget} = 80$ ore è coerente con il caso ottimo del calcolo di Cap.23.6 (72 ore caso ottimo per 8 fold sequenziali); il margine di 8 ore (~11%) copre la varianza wall-clock attesa di c5.4xlarge spot (interruzioni e rilancio, overhead di IO sui log di calibrazione fold-per-fold), permettendo al GA di completare $F = 8$ fold sequenziali del walk-forward nested nel caso ottimo senza rientro in stop forzato. Il valore $T_{budget} = 80$ ore è la chiusura di RP-1 Review v1 (la specifica precedente $T_{budget} = 60$ ore non copriva il caso ottimo 8 fold completi). Il floor di sicurezza inferiore è $T_{budget} = 75$ ore (3 ore di margine sul caso ottimo); la scelta di 80 ore privilegia il margine di robustezza sul margine di costo compute (~4 ore aggiuntive di EC2 spot).
...
| $T_{budget}$ — budget compute wall-clock | ore | 80 | Cap.26.2 PV | Coerente Cap.4 PI / Cap.23.6 PV; copre caso ottimo $F = 8$ fold sequenziali (~72 ore) con margine ~11% (chiusura RP-1 Review v1) |
...
| $K_{max}$ — cardinalità feature attive survival (per strato sotto stratificazione Cap.25.5) | intero positivo | 6 | Cap.22.6 PV; Cap.26.7 PV | Harrell 2015 rule $N_{eventi,\text{strato}}/K \geq 10$ con $N_{eventi,\text{strato}} \geq 60$ sotto split 50/50 |
